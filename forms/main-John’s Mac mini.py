...
#itemID = '521'
#currentVersion = '01.05'
#productName = 'NodeEra Local'   # also change in nodeera.py
#menuAccess = "LocalOnly"

# set PRO license and product info here
licenseURL = "www.noderapro.com:80"
itemID = '1459'
currentVersion = '01.05'           # also change in nodeera.py
productName = 'NodeEra Pro'   # also change in nodeera.py
menuAccess = "Pro"
...
#                    neoCon = NeoCon(name=lastNeoConName, type="Connection")
    ...
    def initSettings(self, ):
        ...
        try:
            ...
            if self.expirationDate is None:
                ...
                self.settings.setValue("License/expirationDate",self.expirationDate)
        except:
            self.expirationDate = 'No expiration date set'
            self.settings.setValue("License/expirationDate",self.expirationDate)
...
#                self.defaultLoggingPath = '//'
#                self.settings.setValue("Default/LoggingPath",self.defaultLoggingPath)
...
#            self.settings.setValue("Default/LoggingPath",self.defaultProjPath)
            self.logDir = os.getcwd()
            self.logDir = os.path.realpath(os.path.abspath(self.logDir))
            self.settings.setValue("Default/LoggingPath",self.logDir)            
...
#                self.defaultProjPath = '//'
#                self.settings.setValue("Default/ProjPath",self.defaultProjPath)
...
#            self.settings.setValue("Default/ProjPath",self.defaultProjPath)
        ...
        try:
            ...
            if self.localNeoCon is None:
                self.settings.setValue("NeoCon/connection/LOCAL", NeoCon().localNeoConDict())
        except:
            self.settings.setValue("NeoCon/connection/LOCAL", NeoCon().localNeoConDict())
...
#####################################################################
    ...
    def on_actionOpenProject_triggered(self):
        ...
        """
        self.on_actionSaveProject_triggered()
        dlg = QFileDialog()
    ...
    @pyqtSlot()
    def on_actionRecent_Projects_triggered(self):
        """
        User selects recent projects menu item
        """
        pass
    ...
    def on_actionSaveProject_triggered(self):
        ...
        if not curPageWidget is None:
            if curPageWidget.tabPage.currentWidget().pageType == "PROJECT":
                ...
                self.helper.displayErrMsg("Save Project","{} not a project".format(curPageWidget.tabPage.currentWidget().pageType))
        ...
        for ctr in range(0, curPageWidget.tabPage.count()):
            ...
            if tab.pageType == "PROJECT":  
                if (not fileName is None) and (tab.fileName == fileName):
                    ...
                    return
#                print("currently open project {}".format(tab.fileName))
...
#        if projectTab.fileName is None:
